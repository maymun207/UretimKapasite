...
import { DiscreteInputs, BottleneckStep, CostInputs, INDUSTRY_EXTRA_FIELDS } from "@/lib/calculations";
import AISuggestButton, { AISuggestions } from "./AISuggestButton";

/* ── Industry keys (must match en.json / tr.json) ────────────── */
const INDUSTRIES = [
    "automotive", "plastic", "packaging", "textile", "metal", "electronics",
];
...
interface DiscreteFormProps {
    ...
    onCostCalculate: (inputs: DiscreteInputs, costInputs: CostInputs) => void;
    country?: string;
    onBack: () => void;
}

export default function DiscreteForm({ onCalculate, onCostCalculate, country, onBack }: DiscreteFormProps) {
    ...
    // Cost parameters
    const [rawMaterialCost, setRawMaterialCost] = useState(2.5);
    ...
    const getCapacityInputs = (): DiscreteInputs => ({
        lines, shifts, hoursPerShift, cycleTime, outputRate, availability, efficiency, yieldRate, workingDaysPerWeek, stations,
    });
        ...
        >
            ...
            <div className="glass-card rounded-2xl p-6 md:p-8 mb-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                    <NumberField label={t("cost.rawMaterial")} tooltip={t("cost.rawMaterialTooltipDiscrete")} value={rawMaterialCost} onChange={setRawMaterialCost} min={0} step={0.1} unit={t("cost.rawMaterialUnitDiscrete")} />
